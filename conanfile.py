...
class PopplerConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.with_qt:
            ...
            self.requires("zstd/1.4.5") # FIXME: REMOVE!
    ...
    def build(self):
        tools.save(os.path.join(self.install_folder, "FindFreetype.cmake"),
                   textwrap.dedent("""
                    set(FREETYPE_INCLUDE_DIRS ${Freetype_INCLUDE_DIRS})
                    set(FREETYPE_LIBRARIES Freetype::Freetype)
                    """), append=True)
        if self.options.get_safe("with_libiconv"):
            tools.save(os.path.join(self.install_folder, "FindIconv.cmake"),
                       textwrap.dedent("""
                        set(ICONV_FOUND 1)
                        ...
                        """), append=True)
        if self.options.with_openjpeg:
            tools.save(os.path.join(self.install_folder, "FindOpenJPEG.cmake"),
                       "set(OPENJPEG_MAJOR_VERSION {})\n".format(tools.Version(self.deps_cpp_info["openjpeg"].version).major), append=True)
            tools.save(os.path.join(self.install_folder, "FindOpenJPEG.cmake"),
                       textwrap.dedent("""
                        set(OPENJPEG_INCLUDE_DIRS ${OpenJPEG_INCLUDE_DIRS})
                        if(NOT TARGET openjp2)
                          add_library(openjp2 INTERFACE)
                          target_link_libraries(openjp2 INTERFACE OpenJPEG::OpenJPEG)
                        endif()
                        """), append=True)
        if self.options.with_lcms:
            # os.rename("Findlcms2.cmake", "FindLCMS2.cmake")
            tools.save(os.path.join(self.install_folder, "FindLCMS2.cmake"),
                       textwrap.dedent("""
                        include(${CMAKE_CURRENT_LIST_DIR}/Findlcms.cmake)
                        set(LCMS2_FOUND ON)
                        set(LCMS2_INCLUDE_DIR ${lcms_INCLUDE_DIR})
                        set(LCMS2_INCLUDE_DIRS ${lcms_INCLUDE_DIRS})
        ...
        tools.save(os.path.join(self.build_folder, "CMakeLists.txt"), textwrap.dedent("""\
            ...
            list(APPEND CMAKE_MODULE_PATH "{install_folder}")
            add_subdirectory("{source_folder}" poppler)
        """).format(
        ...
        cmake.definitions["BUILD_QT6_TESTS"] = False
        cmake.definitions["BUILD_CPP_TESTS"] = False

        cmake.definitions["ENABLE_UTILS"] = False
        cmake.definitions["ENABLE_CPP"] = self.options.cpp

        cmake.definitions["ENABLE_SPLASH"] = self.options.splash
        cmake.definitions["FONT_CONFIGURATION"] = self.options.fontconfiguration
        cmake.definitions["ENABLE_JPEG"] = self.options.with_libjpeg
        cmake.definitions["WITH_PNG"] = self.options.with_png
        cmake.definitions["WITH_TIFF"] = self.options.with_tiff
        cmake.definitions["WITH_NSS3"] = self.options.with_nss
        cmake.definitions["WITH_Cairo"] = self.options.with_cairo
        cmake.definitions["ENABLE_GLIB"] = self.options.get_safe("with_glib", False)
        cmake.definitions["ENABLE_GOBJECT_INTROSPECTION"] = self.options.get_safe("with_gobject_introspection", False)
        cmake.definitions["WITH_GTK"] = self.options.get_safe("with_gtk", False)
        cmake.definitions["WITH_Iconv"] = self.options.get_safe("with_libiconv")
        cmake.definitions["ENABLE_ZLIB"] = self.options.with_zlib
        cmake.definitions["ENABLE_LIBOPENJPEG"] = "openjpeg2" if self.options.with_openjpeg else "none"
        cmake.definitions["ENABLE_CMS"] = "lcms2" if self.options.with_lcms else "none"
        cmake.definitions["ENABLE_LIBCURL"] = self.options.with_libcurl

        cmake.definitions["POPPLER_DATADIR"] = self.deps_user_info["poppler-data"].datadir.replace("\\", "/")
        cmake.definitions["FONT_CONFIGURATION"] = self.options.fontconfiguration
        cmake.definitions["BUILD_CPP_TESTS"] = False
        cmake.definitions["ENABLE_GTK_DOC"] = False
        cmake.definitions["ENABLE_QT5"] = self.options.with_qt and tools.Version(self.deps_cpp_info["qt"].version).major == "5"
        cmake.definitions["ENABLE_QT6"] = self.options.with_qt and tools.Version(self.deps_cpp_info["qt"].version).major == "6"
        cmake.definitions["ENABLE_LIBOPENJPEG"] = "openjpeg2" if self.options.with_openjpeg else "none"
        if self.options.with_openjpeg:
            # FIXME: openjpeg's cmake_find_package should provide these variables
            cmake.definitions["OPENJPEG_MAJOR_VERSION"] = self.deps_cpp_info["openjpeg"].version.split(".", 1)[0]
        cmake.definitions["ENABLE_CMS"] = "lcms2" if self.options.with_lcms else "none"
        cmake.definitions["ENABLE_DCTDECODER"] = self._dct_decoder
        cmake.definitions["USE_FLOAT"] = self.options.float
        cmake.definitions["RUN_GPERF_IF_PRESENT"] = False
        cmake.definitions["ENABLE_RELOCATABLE"] = True
